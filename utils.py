...
def calc_total_expected_reward(current_worker_location, current_trailer_location, action, transitions, action_results, states):
    # find direct costs of the action (moving the trailer)
    cost = 0
    future_trailer_loc = action_results[action]
    cost += calc_cost_move_trailer(current_loc=current_trailer_location, future_loc= future_trailer_loc)
    ...
    for i in range(len(transitions)):
        ...
        cost += calc_cost_use_trailer(worker_loc=states[i], trailer_loc=future_trailer_loc, states = states)*transitions[current_worker_location][i]
    return cost
